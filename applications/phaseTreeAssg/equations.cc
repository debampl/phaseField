  ...
  [[maybe_unused]] const VectorizedArray<double> element_volume) const
{
  ...
  scalarvalueType faV  = A0*c*c*(1.0 - c)*(1.0 - c); //+ A1 * c + A2 * c * c + A3 * c * c * c + A4 * c * c * c * c);
  scalarvalueType facV = A0*c*(1.0 - c)*(0.5 - c); //(A1 + 2.0 * A2 * c + 3.0 * A3 * c * c + 4.0 * A4 * c * c * c);
  scalarvalueType faccV = A0*(0.5 - c*c); //(2.0 * A2 + 6.0 * A3 * c + 12.0 * A4 * c * c);
  scalarvalueType fbV   = 0.0; //(B2 * c * c + B1 * c + B0);
  scalarvalueType fbcV  = 0.0; //(2.0 * B2 * c + B1);
  scalarvalueType fbccV = 0.0; //constV(2.0 * B2);
  scalarvalueType h1V =
  ...
  double C11_BCC = 44.62;
  double C12_BCC = 13.384;
  double C44_BCC = 15.615;
  ...
  scalarvalueType aplhaC  = c*c*c*(10.0 - 15.0*c + 6.0*c*c) - 0.5; 


  // Define CIJ_from this perticular example from easy use
  for (unsigned int i = 0; i < 2 * dim - 1 + dim / 3; i++)
        {
          for (unsigned int j = 0; j < 2 * dim - 1 + dim / 3; j++)
            {
              CIJ_combined[i][j] = 0.5*CIJ_BCC[i][j] + aplhaC*CIJ_B2[i][j];
      ...
      for (unsigned int i = 0; i < 2 * dim - 1 + dim / 3; i++)
        {
          for (unsigned int j = 0; j < 2 * dim - 1 + dim / 3; j++)
            {
              CIJ_combined[i][j] = CIJ_combined[i][j];
  ...
  [[maybe_unused]] const VectorizedArray<double> element_volume) const
{
  ...
  /// I have hard coded the materail paperter to be used.
  /// These papramters are calculated from the matlab code attached MaterailProp.m
