...
variableAttributeLoader::loadVariableAttributes()
{
  ...
  set_dependencies_gradient_term_RHS(
    0,
   "c, grad(c), n2, grad(n2), n3, grad(n3), grad(u), hess(u)");
  ...
  set_dependencies_value_term_RHS(1, "c, n2, n3, grad(u)");
  set_dependencies_gradient_term_RHS(1, "grad(n2)");
  ...
  set_dependencies_value_term_RHS(2, "c, n2, n3, grad(u)");
  set_dependencies_gradient_term_RHS(2, "grad(n3)");
  ...
  set_dependencies_gradient_term_RHS(3, "c, n2, n3, grad(u)");
  set_dependencies_value_term_LHS(3, "");
  set_dependencies_gradient_term_LHS(3, "n2, n3, grad(change(u))");
}
  ...
  [[maybe_unused]] const VectorizedArray<double> element_volume) const
{
  ...
  scalarvalueType n2  = variable_list.get_scalar_value(1);
  scalargradType  n2x = variable_list.get_scalar_gradient(1);

  // The third order parameter and its derivatives
  scalarvalueType n3  = variable_list.get_scalar_value(2);
  scalargradType  n3x = variable_list.get_scalar_gradient(2);
  ...
  if (c_dependent_misfit == true)
    {
      uxx = variable_list.get_vector_hessian(3);
    }
  ...
  if (c_dependent_misfit == true)
    {
      ...
      //     for (unsigned int j = 0; j < dim; j++)
      //       {
      //         E3i][j] = -(sfts1c[i][j] * h1V + sfts2c[i][j] * h2V + sfts3c[i][j] * h3V);
      //       }
      //   }
  ...
  variable_list.set_scalar_value_term_RHS(1, eq_n2);
  variable_list.set_scalar_gradient_term_RHS(1, eqx_n2);

  variable_list.set_scalar_value_term_RHS(2, eq_n3);
  variable_list.set_scalar_gradient_term_RHS(2, eqx_n3);
}
  ...
  [[maybe_unused]] const VectorizedArray<double> element_volume) const
{
  ...
  // u
  vectorgradType Dux = variable_list.get_change_in_vector_gradient(3);
